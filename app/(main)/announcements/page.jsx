  ...
  return (
    <>
      ...
      {!announcements.length && (
        <p className="text-xl font-xl text-center p-2">No Announcements Yet</p>
      )}
      ...
      <div className="px-2">
        {announcements.map((announcement, index) => (
          <div
            key={index}
            className="border border-borderColor3 rounded my-2 mx-auto max-w-xl"
          >
            <div className="border-b border-borderColor3 bg-backgroundColor2 rounded-t p-2 flex flex-row justify-between items-center">
              <p className="text-md text-textColor1">{announcement.title}</p>
              <p className="text-xs block text-textColor2">
                {formatTimestamp(announcement._createdAt)}
              </p>
            </div>
